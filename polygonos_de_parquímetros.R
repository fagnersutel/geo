...
resultado = predict.apcluster(negDistMat(r=2), x2[apres@exemplars, ],  parquimetros[1:length(parquimetros$V3), 2:3])
parquimetros$cluster[1:length(parquimetros$V3)] = resultado
length(resultado)
head(parquimetros)
...
leaflet(nycounties) %>%
  addTiles() %>%
  addPolygons(stroke = FALSE, smoothFactor = 0.8, fillOpacity = 0.9, fillColor = ~pal(clusters_encontrados), popup=nycounties)
